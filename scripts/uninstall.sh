...
isScriptLoaded="$(qdbus6 org.kde.KWin /Scripting org.kde.kwin.Scripting.isScriptLoaded "${package_name}")"
if [ "${isScriptLoaded}" = "true" ]; then
  echo "Unloading ${package_name}"
  qdbus6 org.kde.KWin /Scripting org.kde.kwin.Scripting.unloadScript "${package_name}" > /dev/null
fi
...
if kpackagetool6 --type KWin/Script --list | grep ${package_name} > /dev/null; then
  ...
  qdbus6 org.kde.kglobalaccel /component/kwin org.kde.kglobalaccel.Component.cleanUp > /dev/null
